...
1. [`star`] with a width of 2 columns.
1. [`needsAttention`] with a width of 4 columns.
...
1. [`labels`] with a width of 22 columns, hidden by default.
1. [`numComments`] with a width of 3 columns.
1. [`reviewStatus`] with a width of 3 columns.
1. [`reviewStatusBot`] with a width of 3 columns.
1. [`ci`] with a width of 3 columns.
1. [`mergeStatus`] with a width of 3 columns.
...
| :------- | :--- | :------------------------------------------------- |
| `star`   | yaml | <Code code={`width: 2`} lang="yaml" frame="none"/> |

This column displays a star icon for PRs you've starred, and is blank
otherwise. Starring is local to your machine and doesn't call the GitHub
API; toggle it with the `star` keybinding.

This column has no heading text.

## PR Needs Attention Column

| Property         | Type | Default                                            |
| :--------------- | :--- | :------------------------------------------------- |
| `needsAttention` | yaml | <Code code={`width: 4`} lang="yaml" frame="none"/> |

This column displays a 👀 icon when the most recent activity on a PR was
authored by someone other than the current user, indicating the PR may need
your attention.

When enriched data is available (after selecting a PR), the column considers
both comments and reviews by timestamp and shows the indicator if the most
recent one was authored by someone else. Before enrichment, only the last
review is checked, so comment-only activity may not be reflected until the
PR is selected.
...
This column displays the number of unresolved review threads on a PR. It is
blank when the PR has no unresolved review threads (either because it has
none, or because they've all been resolved).
...
Review status is split into two independently configurable columns: one for
human reviewers and one for bot (GitHub App) reviewers. Each column's
status is computed only from reviews left by accounts of its own type, so a
PR with an approving human and a commenting bot (or vice versa) shows a
different icon in each column.

| Property          | Type | Default                                            |
| :---------------- | :--- | :------------------------------------------------- |
| `reviewStatus`    | yaml | <Code code={`width: 3`} lang="yaml" frame="none"/> |
| `reviewStatusBot` | yaml | <Code code={`width: 3`} lang="yaml" frame="none"/> |

Both columns display the review status of their group as an icon:

- When the group hasn't reviewed, the icon is <NerdFontIcon icon="nf-fa-clock" /> and the
  color is the value of [`theme.colors.text.faint`].
- When the group has approved, the icon is <NerdFontIcon icon="nf-cod-check" /> and the color is
  the value of [`theme.colors.text.primary`].
- When the group has requested changes, the icon is <NerdFontIcon icon="nf-md-keyboard_return" /> and the
  color is the value of [`theme.colors.text.warning`].
- When the group has only commented, the icon is the comment glyph, colored with
  [`theme.colors.text.primary`].

The heading for the `reviewStatus` (human) column is

<NerdFontIcon icon="nf-md-account_check_outline" />. The heading for the
`reviewStatusBot` column is <NerdFontIcon icon="nf-md-robot" />.

Set `reviewStatusBot.hidden: true` to hide the bot column while keeping the
human column visible (or vice versa) - the two columns' visibility isn't
linked.
...
## PR Merge Status Column

| Property      | Type | Default                                            |
| :------------ | :--- | :------------------------------------------------- |
| `mergeStatus` | yaml | <Code code={`width: 3`} lang="yaml" frame="none"/> |

This column displays whether GitHub can determine that a PR is mergeable. It uses only
GitHub's conflict-focused `mergeable` value; branch freshness, review requirements,
unresolved threads, and CI results do not affect this column.

- When the PR has merge conflicts, a warning icon is shown in the error color.
- When the PR is mergeable, a check icon is shown in the success color.
- While GitHub is still calculating mergeability, a warning icon is shown in the warning color.

The heading for this column is ``.

[`theme.colors.text.error`]: /configuration/theme#error-text-color
[`theme.colors.text.success`]: /configuration/theme#success-text-color
[`theme.colors.text.warning`]: /configuration/theme#warning-text-color
...
| Property | Type | Default                                                           |
| :------- | :--- | :---------------------------------------------------------------- |
| `labels` | yaml | <Code code={`width: 22\nhidden: true`} lang="yaml" frame="none"/> |

This column displays the labels assigned to a PR as colored pills.
Up to 6 labels per PR are fetched and displayed.

The heading for this column is <NerdFontIcon icon="nf-md-label_multiple_outline"/>.
